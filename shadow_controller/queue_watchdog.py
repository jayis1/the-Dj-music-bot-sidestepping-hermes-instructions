...
class QueueWatchdog:
    ...
    async def start(self):
        ...
        logger.info(
            ...
            self.interval, self.min_songs, self.guild_id,
        )
    ...
    async def _check_and_refill(self):
        ...
        logger.debug(
            ...
            queue_length, "1" if playing else "0", effective_count, autodj,
        )
        ...
        logger.warning(
            ...
            effective_count, self.min_songs, current_title[:50] if current_title else "nothing playing",
        )
            ...
            try:
                ...
                if playlist_url and playlist_url != self._last_playlist_url:
                    ...
                    if not result.get("error"):
                        ...
                        await self.alerts.playlist_queued(playlist_url, source="auto-discovery")
                    else:
