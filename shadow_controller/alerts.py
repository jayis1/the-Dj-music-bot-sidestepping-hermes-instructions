...
class AlertSystem:
    ...
    async def send(
        ...
        logger.log(
            logging.INFO if level in (AlertLevel.INFO, AlertLevel.SUCCESS) else logging.WARNING,
        ...
        try:
            ...
            async with self._session.post(
                ...
                if resp.status == 204:
                    ...
                    logger.warning("Webhook rate limited, retry after %.1fs", retry_after)
                    await asyncio.sleep(retry_after)
                    # Retry once
                    async with self._session.post(self.webhook_url, json=payload) as resp2:
                        if resp2.status != 204:
                            logger.error("Webhook retry failed: %d", resp2.status)
                else:
                    logger.error("Webhook failed: %d %s", resp.status, await resp.text()[:200])
