...
Genres: Lo-fi, Rap, Electro Swing, EDM, Chill Beats, Reggae
"""
...
class PlaylistFinder:
    ...
    def __init__(self, config: dict, api_client, browser_manager, alert_system):
        ...
        self.genres = config.get("genres", ["lo-fi", "rap", "electro_swing", "edm", "chill_beats"])
    ...
    async def start(self):
        ...
        logger.info(
            ...
            self.interval, self.genres, self._hermes_available,
        )
                    ...
                    if resp.status == 200:
                        ...
                        if hermes_models:
                            ...
                            if models:
                                ...
                                self._hermes_available = True
                                logger.info("No Hermes model, using fallback: %s", self.ollama_model)
                            else:
    ...
    async def find_playlist(self) -> Optional[str]:
        ...
        unused = [p for p in self._discovered_playlists if p not in self._used_playlists]
        if unused:
        ...
        if self._hermes_available:
            try:
                ...
                if playlists:
                    ...
                    logger.info("Hermes found %d playlists for '%s'", len(playlists), genre)
                    return
            except Exception as e:
...
If none of the results are good playlists, reply with "NONE"."""
        ...
        try:
            async with aiohttp.ClientSession() as session:
                payload = {
                    ...
                    "messages": [
                        {"role": "system", "content": "You are a music curator AI for a 24/7 radio station. You only output YouTube playlist URLs, nothing else."},
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        urls = re.findall(r'https?://(?:www\.)?youtube\.com/playlist\?list=[\w-]+', content)
                        if not urls:
                            # Also check for /watch URLs with list= param
                            urls = re.findall(r'https?://(?:www\.)?youtube\.com/watch\?[^"\s]*list=([\w-]+)', content)
                            urls = [f"https://www.youtube.com/playlist?list={lid}" for lid in urls]
                        return urls
                    else:
    ...
    async def _discover_via_search(self, genre: str) -> list:
        ...
        try:
            ...
            urls = [f"https://www.youtube.com/playlist?list={pid}" for pid in playlist_ids]
