...
class BrowserManager:
    ...
    def __init__(self, config: dict):
        ...
        """
        self.config = config
        self.firefox_profile = config.get("firefox_profile_path") or _find_firefox_profile()
    ...
    async def start(self):
        ...
        if self.firefox_profile and os.path.isdir(self.firefox_profile):
            ...
            logger.warning("Launched Firefox WITHOUT existing profile — YouTube not logged in!")
        ...
        if self.youtube_live_url:
            ...
            await self._stream_page.goto(self.youtube_live_url, wait_until="networkidle")
    ...
    async def read_cookie_txt(self) -> Optional[str]:
        ...
        try:
            ...
            if "# Netscape" in content or content.strip().startswith("."):
                ...
                logger.warning("cookies.txt at %s doesn't look like Netscape format", path)
    ...
    async def extract_cookies_from_browser(self) -> Optional[str]:
        ...
        try:
            ...
            netscape_lines = ["# Netscape HTTP Cookie File", "# https://curl.se/docs/http-cookies.html", ""]
            ...
            content = "\n".join(netscape_lines)
            logger.info("Extracted %d YouTube cookies from browser context", len(yt_cookies))
