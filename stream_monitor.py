...
class StreamMonitor:
    ...
    async def start(self):
        ...
        logger.info(
            ...
            self.interval, self.should_be_live,
        )
    ...
    async def _check_stream_health(self):
        ...
        is_curated = stream_status.get("curated", False)
        
        logger.debug(
            "Stream: active=%s, running=%s, title=%s",
            is_active, is_running, stream_title[:40] if stream_title else "none",
        )
        
        # If stream should be live but isn't, try to restart
        if self.should_be_live and not is_running and is_active:
            await self._attempt_stream_recovery("stream not running but active flag set")
        ...
        logger.debug(
            ...
            obs_connected, obs_streaming, obs_recording,
        )
        ...
        # OBS connected but not streaming when it should be
            ...
            if result.get("error"):
                ...
                await self.alerts.success("OBS streaming restarted", key="obs_stream_restarted")
    ...
    async def _attempt_stream_recovery(self, reason: str):
        ...
        if now - self._last_restart_time < self.restart_cooldown:
            logger.debug("Stream restart on cooldown (%.0fs remaining)", 
        ...
        logger.warning("Stream recovery attempt %d/%d: %s", 
                       self._restart_attempts, self.max_restart_attempts, reason)
