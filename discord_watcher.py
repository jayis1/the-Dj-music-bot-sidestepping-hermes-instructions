...
YOUTUBE_PATTERNS = [
    # Standard watch URLs
    re.compile(r'(https?://(?:www\.)?youtube\.com/watch\?[^\s]*v=[\w-]+[^\s]*)', re.IGNORECASE),
    # Short URLs
    re.compile(r'(https?://youtu\.be/[\w-]+[^\s]*)', re.IGNORECASE),
    # Playlist URLs
    re.compile(r'(https?://(?:www\.)?youtube\.com/playlist\?list=[\w-]+[^\s]*)', re.IGNORECASE),
    # Music URLs
    re.compile(r'(https?://music\.youtube\.com/watch\?[^\s]*v=[\w-]+[^\s]*)', re.IGNORECASE),
    # Shorts
...
class DiscordWatcher:
    ...
    async def start(self):
        ...
        if not self.token:
            logger.warning("No Discord watcher token configured — fan requests disabled")
            return
        ...
        async def on_ready():
            ...
            if channel:
                ...
                logger.warning("Channel ID %s not found or not accessible", self.channel_id)
        ...
        try:
            ...
            await self.alerts.error("Discord watcher login failed — check token", key="discord_login_fail")
        except Exception as e:
            logger.error("Discord watcher error: %s", e)
            await self.alerts.error(f"Discord watcher crashed: {e}", key="discord_crash")
    ...
    async def _handle_message(self, message: discord.Message):
        ...
        logger.info(
            ...
            message.author.name, len(urls), urls[0][:60],
        )
        ...
        for url in urls:
            ...
            try:
                ...
                if result.get("error"):
                    logger.warning("Failed to queue fan request %s: %s", url[:50], result.get("error"))
                    await self.alerts.error(
                        f"Fan request failed: {url[:50]} — {str(result.get('error', ''))[:50]}",
                        key=f"fan_request_fail:{url[:20]}",
                    )
                else:
                    logger.info("Fan request queued: %s from @%s", url[:60], message.author.name)
    ...
    async def scan_channel_history(self, limit: int = 50):
        ...
        channel = self._bot.get_channel(self.channel_id)
        if not channel:
            logger.warning("Cannot scan history — channel %d not found", self.channel_id)
            return
        
        logger.info("Scanning last %d messages in #%s for missed fan requests...", limit, channel.name)
        ...
        logger.info("Scan complete: %d unique YouTube URLs found (added to dedup, not auto-queued)", count)