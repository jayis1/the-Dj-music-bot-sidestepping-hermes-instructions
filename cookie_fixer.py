...
class CookieFixer:
    ...
    async def start(self):
        ...
        self._running = True
        logger.info("Cookie fixer started (interval: %ds, max age: %d days)", self.interval, self.max_age_days)
    ...
    async def _check_and_fix(self):
        ...
        if cookie_age_days > self.max_age_days:
            needs_refresh = True
            reason = f"cookies {cookie_age_days:.1f} days old (max: {self.max_age_days})"
        ...
        if not cookie_text:
            await self.alerts.cookie_refresh_failed("could not extract cookies from browser")
            self._consecutive_failures += 1
            return
        ...
        logger.info("Cookies injected: %s", result.get("message", result.get("status", "ok")))
        ...
        if auth_verify.get("auth_blocked"):
            await self.alerts.cookie_refresh_failed("auth still blocked after injection")
            self._consecutive_failures += 1
        else:
    ...
    async def _get_fresh_cookies(self) -> Optional[str]:
        ...
        if cookie_text and len(cookie_text.strip()) > 50:
            ...
            if found >= 2:
                logger.info("Got fresh cookies from cookie.txt plugin (%d/%d essential found)", found, len(essential_cookies))
                return cookie_text
            else:
                logger.warning("cookie.txt found but missing essential YouTube cookies (%d/%d)", found, len(essential_cookies))
